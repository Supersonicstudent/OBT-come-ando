...
def infer_and_process(model, data_loader, device):
    ...
    with torch.no_grad():  # Desliga o cálculo do gradiente para a inferência
        for images, _ in data_loader:  # Assume que data_loader fornece imagens e rótulos
            images = images.to(device)  # Move as imagens para o dispositivo (CPU ou GPU)
            predictions = model(images)  # Faz a inferência
...
# Supondo que model e data_loader estejam definidos
# model = UNet().to(device)  # Inicialize o modelo e mova para o dispositivo
# data_loader = ...  # Defina seu DataLoader aqui

# Exemplo de chamada da função
# suggestions = infer_and_process(model, data_loader, device)
