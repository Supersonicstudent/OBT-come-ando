...
# Defina as origens e destinos corretos
origem_geral = "Brasília, Brazil"
...
directions_result = gmaps.directions(
    ...
    destino_geral,
    mode="driving",
    departure_time=datetime(year=2024, month=7, day=10, hour=19, minute=0)
)
...
print("Endereço inicial:", start_address)
print("Endereço final:", end_address)
...
def get_congestion_addresses(origem_geral, destino_geral):
    # Realiza a solicitação de direções usando as variáveis fornecidas
    directions_result = gmaps.directions(
        origem_geral,
        destino_geral,
        mode="driving",
    ...
    start_location = steps_data[0]['start_location']
    end_location = steps_data[-1]['end_location']
    
    start_address = gmaps.reverse_geocode((start_location['lat'], start_location['lng']))[0]['formatted_address']
    end_address = gmaps.reverse_geocode((end_location['lat'], end_location['lng']))[0]['formatted_address']
    
    return start_address, end_address

# Exemplo de uso:
start_address, end_address = get_congestion_addresses(origem_geral, destino_geral)
print("Endereço inicial:", start_address)
print("Endereço final:", end_address)
