...
def process_data():
    ...
    return {"start_location": start_location_var, "end_location": end_location_var, "start_address": start_address, "end_address": end_address}
...
def segment_image(image_path, output_path):
    ...
    # Definindo o modelo para modo de avaliação
    model.eval()
    with torch.no_grad():
    ...
    segmented_image = Image.fromarray(output_predictions.astype('uint8') * 255)
    segmented_image.save(output_path)

# Segmentando a imagem de teste
segment_image(test_image_path, output_segmented_path)
